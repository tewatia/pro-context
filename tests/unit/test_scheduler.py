"""Unit tests for the registry update scheduler in server.py.
...
class TestSchedulerStdioMode:
    ...
    async def test_stdio_runs_once(self) -> None:
...
class TestSchedulerHttpMode:
    ...
    async def test_transient_failure_uses_backoff(self) -> None:
        ...
        with (
            patch("procontext.server.check_for_registry_update", mock_check),
            patch("asyncio.sleep", side_effect=fake_sleep),
            patch("procontext.server._jittered_delay", side_effect=lambda seconds: float(seconds)),
            pytest.raises(asyncio.CancelledError),
        ):
            await _run_registry_update_scheduler(state)
